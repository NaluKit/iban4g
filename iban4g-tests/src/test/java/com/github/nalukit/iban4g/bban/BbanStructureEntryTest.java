 ...
 */
package com.github.nalukit.iban4g.bban;
...
public class BbanStructureEntryTest {
  ...
  private static void assertSeededRandomBbanStructureEntryEquals(BbanStructureEntry entry,
                                                                 String expected,
                                                                 int seed) {
    String generated = entry.getRandom(new Random(seed));
    assertEquals("expect that creating " + entry + " with seed '" + seed + "' is deterministic",
                 expected,
                 generated);
  }

  private static String getDistinctSortedChars(String s) {
    Stream<Character> chars = CharBuffer.wrap(s.toCharArray())
                                        .chars()
                                        .mapToObj(ch -> (char) ch);
  ...
  public void expectRandomAccountNumberIsDeterministicWhenSeeded() {
    BbanStructureEntry entry = BbanStructureEntry.accountNumber(10,
                                                                'a');

    assertSeededRandomBbanStructureEntryEquals(entry,
                                               "RAHJMYUWWK",
                                               1);
    assertSeededRandomBbanStructureEntryEquals(entry,
                                               "SGAVREIZNE",
  ...
  public void expectRandomOwnerAccountNumberIsDeterministicWhenSeeded() {
    BbanStructureEntry entry = BbanStructureEntry.ownerAccountNumber(11,
                                                                     'n');
  ...
  public void expectRandomBankCodeIsDeterministicWhenSeeded() {
    BbanStructureEntry entry = BbanStructureEntry.bankCode(12,
                                                           'c');
  ...
  public void expectRandomAlphanumericBranchCodeIsValid() {
    for (int i = 0; i < 100; i++) {
      BbanStructureEntry entry = BbanStructureEntry.branchCode(5,
                                                               'c');

      String generated = entry.getRandom();

      assertTrue("expect '" + generated + "' is alphabetic. (" + entry + ")",
  ...
  public void expectRandomAlphabeticNationalCheckDigitIsValid() {
    for (int i = 0; i < 100; i++) {
      BbanStructureEntry entry = BbanStructureEntry.nationalCheckDigit(6,
                                                                       'a');

      String generated = entry.getRandom();

      assertTrue("expect '" + generated + "' only has capitalized letters. (" + entry + ")",
  ...
  public void expectRandomNumericIdentificationNumberIsValid() {
    for (int i = 0; i < 100; i++) {
      BbanStructureEntry entry = BbanStructureEntry.identificationNumber(7,
                                                                         'n');

      String generated = entry.getRandom();

      assertTrue("expect '" + generated + "' only has numbers. (" + entry + ")",
  ...
  public void expectZeroLengthEntryGeneratesZeroLengthStrings() {
    BbanStructureEntry entry = BbanStructureEntry.accountType(0,
                                                              'n');
    for (int i = 0; i < 100; i++) {
      String generated = entry.getRandom();

      assertEquals("expect that entry with length=0 generates zero-length strings. (" + entry + ")",
                   "",
                   generated);
    }
  }
  ...
  public void expectAlphabeticEntryGeneratesAllUppercaseLetters() {
    ...
    BbanStructureEntry entry     = BbanStructureEntry.accountNumber(10_000,
                                                                    'a');
    String             generated = entry.getRandom();
