...
import de.gishmo.gwt.iban4g.shared.bban.BbanEntryType;
import de.gishmo.gwt.iban4g.shared.bban.BbanStructure;
import de.gishmo.gwt.iban4g.shared.bban.BbanStructureEntry;

import static de.gishmo.gwt.iban4g.shared.IbanFormatException.IbanFormatViolation.*;
  ...
  private static void validateBbanLength(final String iban,
    ...
    if (expectedBbanLength != bbanLength) {
      throw new IbanFormatException(BBAN_LENGTH,
                                    ...
                                    StringUtils.format("[%s] length is %d, expected BBAN length is: %d",
                                                       bban,
                                                       bbanLength,
                                                       expectedBbanLength));
    }
  ...
  private static void validateCheckDigit(final String iban) {
    if (calculateMod(iban) != 1) {
      ...
      throw new InvalidCheckDigitException(
                                            checkDigit,
                                            expectedCheckDigit,
                                            StringUtils.format("[%s] has invalid check digit: %s, " +
      ...
      try {
        ...
        if (numericValue < 0 || numericValue > 35) {
          throw new IbanFormatException(IBAN_VALID_CHARACTERS,
                                        ...
                                        StringUtils.format("Invalid Character[%d] = '%d'",
                                                           i,
                                                           numericValue));
        }
