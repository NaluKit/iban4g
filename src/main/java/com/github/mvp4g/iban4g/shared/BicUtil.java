...
public class BicUtil {
  ...
  public static void validate(final String bic)
    ...
    try {
      ...
      throw new BicFormatException(BicFormatException.BicFormatViolation.UNKNOWN,
                                   e.getMessage());
    }
  }
  ...
  private static void validateEmpty(final String bic) {
    if (bic == null) {
      throw new BicFormatException(BicFormatException.BicFormatViolation.BIC_NOT_NULL,
                                   "Null can't be a valid Bic.");
    }

    if (bic.length() == 0) {
      throw new BicFormatException(BicFormatException.BicFormatViolation.BIC_NOT_EMPTY,
  ...
  private static void validateLength(final String bic) {
    if (bic.length() != BIC8_LENGTH && bic.length() != BIC11_LENGTH) {
      throw new BicFormatException(BicFormatException.BicFormatViolation.BIC_LENGTH_8_OR_11,
                                   StringUtils.format("Bic length must be %d or %d",
                                                      BIC8_LENGTH,
                                                      BIC11_LENGTH));
    }
  }
  ...
  private static void validateCase(final String bic) {
    if (!bic.equals(bic.toUpperCase())) {
      throw new BicFormatException(BicFormatException.BicFormatViolation.BIC_ONLY_UPPER_CASE_LETTERS,
  ...
  private static void validateCountryCode(final String bic) {
    ...
    if (countryCode.trim()
      ...
      throw new BicFormatException(BicFormatException.BicFormatViolation.COUNTRY_CODE_ONLY_UPPER_CASE_LETTERS,
                                   countryCode,
